  ...
  return (
    <>
      ...
      <mesh position={[0, -0.02, 0]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={0}>
        ...
        <meshBasicMaterial color="#020303" side={THREE.FrontSide} />
      </mesh>
      {/* Fine 1" grid */}
...
const VISUAL_ROTATE_180_MODELS = new Set(["aurora.glb", "thunderbolt.glb", "nial.glb", "battlecrab.glb", "primus.glb", "whitestar.glb"]);
...
function finalForwardPositionOverlapsBase(
  moving: { id: number; hexQ: number; hexR: number; baseRadiusInches?: number | null },
  others: Array<{ id: number; hexQ: number; hexR: number; isDestroyed: boolean; baseRadiusInches?: number | null }>,
...
function GameUnit3D({ unit, isSelected, onClick, onCameraFocus, myUserId, weapons, dragOffset, previewHeadingDelta = 0, phaseViable, firingArc, arcColorScheme = "classic" }: {
      ...
      <group position={[0, 2, 0]} rotation={[0, headingRad, 0]}>
        <ModelErrorBoundary color={modelTint}>
          <Suspense fallback={<ShipModelFallback color={modelTint} />}>
            <BoardModelVisual filename={unit.modelFilename} tint={modelTint} opacity={hasPreview ? 0.28 : 1} />
          </Suspense>
        </ModelErrorBoundary>
...
function TabletMovementController({
  ...
  return (
    <div
      ref={controllerRef}
      className="pointer-events-auto fixed z-40 flex flex-col items-center gap-2 rounded border border-cyan-300/40 bg-black/72 p-3 shadow-2xl shadow-cyan-950/60 backdrop-blur-md"
...
function StagedUnit3D({
  unit, isSelected, onClick, onPointerDown, arcColorScheme = "classic",
}: {
  unit: StagedUnitData;
  isSelected: boolean;
  onClick: (e: any) => void;
...
export default function GameBoard() {
  ...
  const confirmMovePlan = useCallback(() => {
    ...
    if (movePlan.kind === "forward") {
      ...
      const plannedDistance = clampForwardDistanceToLegalRestingSpot(u, units, v, requestedDistance, Math.max(0, speedCap - led.distance));
  ...
  }, [activateUnit.isPending, movePlan, moveUnit.isPending, selectedMovementUi, selectedRemainingMove, selectedUnitData, units]);
        ...
        <div
          ...
          onPointerMove={e => {
            ...
            if ((movementGesture?.kind === "forward" || (!mobileGameChrome && movePlan?.kind === "forward")) && selectedUnitData && selectedUnitData.ownerId === myUserId) {
              ...
              const distance = clampForwardDistanceToLegalRestingSpot(selectedUnitData, units, v, requested, selectedRemainingMove);
