...
export const SHIP_AI_PROFILES = ["brawler", "jouster", "broadside", "standoff"] as const;
export type ShipAiProfile = typeof SHIP_AI_PROFILES[number];

export const EARTH_ALLIANCE_SHIP_AI_PROFILE_SEEDS: Array<{ name: string; aiProfile: ShipAiProfile }> = [
...
const SHIP_AI_PROFILE_BY_NAME = new Map(
  EARTH_ALLIANCE_SHIP_AI_PROFILE_SEEDS.map(seed => [seed.name.toLowerCase(), seed.aiProfile]),
);
