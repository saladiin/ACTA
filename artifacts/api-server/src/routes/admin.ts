...
function matchesIdentityList(identities: string[], list: Set<string>, emptyMeansAllowed: boolean): boolean {
...
function emailVerificationStatus(email: { verification: { status?: string | null } | null }): string | null {
...
async function deleteGameCascade(gameId: number): Promise<void> {
  await db.transaction(async (tx) => {
    await tx.delete(gameAttackAuditLogsTable).where(eq(gameAttackAuditLogsTable.gameId, gameId));
    await tx.delete(gameMovementAuditLogsTable).where(eq(gameMovementAuditLogsTable.gameId, gameId));
    await tx.delete(gameSpecialActionAuditLogsTable).where(eq(gameSpecialActionAuditLogsTable.gameId, gameId));
    await tx.delete(bugReportsTable).where(eq(bugReportsTable.gameId, gameId));
    await tx.delete(gameChatMessagesTable).where(eq(gameChatMessagesTable.gameId, gameId));
    ...
    if (unitRows.length > 0) {
      await tx.delete(unitCriticalEffectsTable)
        .where(or(...unitRows.map((unit) => eq(unitCriticalEffectsTable.gameUnitId, unit.id))));
    }
  ...
  res.json({
    games: games.map((game) => {
      ...
      return {
        ...game,
        lastActivityAt,
        idleSeconds: Math.max(0, Math.floor((now - lastActivityAt.getTime()) / 1000)),
      };
    }),
  });
});
...
router.get("/admin/users", requireAdmin, async (_req, res): Promise<void> => {
  const allowedUsers = parseIdentityList(process.env.B5_ALLOWED_USERS ?? process.env.B5_ACCESS_ALLOWLIST ?? "");
  const adminUsers = parseIdentityList(process.env.B5_ADMIN_USERS ?? process.env.B5_ADMIN_EMAILS ?? "");
  ...
  res.json({
    ...
    users: result.data.map((user) => {
      ...
      return {
        ...
        primaryEmailVerificationStatus: primaryEmail ? emailVerificationStatus(primaryEmail) : null,
...
router.post("/admin/games/:gameId/archive", requireAdmin, async (req, res): Promise<void> => {
  ...
  const days = clampArchiveDays((req.body as { days?: unknown } | undefined)?.days);
  const now = new Date();
  const archiveExpiresAt = new Date(now.getTime() + days * 24 * 60 * 60 * 1000);
  ...
  res.json({ gameId: game.id, archivedAt: game.archivedAt, archiveExpiresAt: game.archiveExpiresAt });
});
...
router.delete("/admin/games/:gameId", requireAdmin, async (req, res): Promise<void> => {
  ...
  const [game] = await db.select({ id: gamesTable.id }).from(gamesTable).where(eq(gamesTable.id, gameId));
  if (!game) {
    res.status(404).json({ error: "Game not found" });
    return;
  }
