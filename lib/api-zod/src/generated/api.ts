...
export const FireWeaponResponse = zod.object({
  ...
  "interceptedHits": zod.number().describe('Hits cancelled by target\'s Interceptors (dice that met interceptorThreshold).'),
  "interceptorRolls": zod.array(zod.number()).describe('Per-Interceptor-die d6 results. One die per point of target\'s Interceptors rating, rolled only when there are still surviving hits after Dodge. Empty when no interceptor check was made: target has no Interceptors, weapon bypasses (Beam \/ Mini Beam \/ Mass Driver \/ Energy Mine), no hits scored, or all hits already cancelled by dodges.'),
  "interceptorThreshold": zod.number().describe('Minimum d6 result for an interceptor die to cancel a hit (6 when a check was made; 0 when interceptorRolls is empty).'),
